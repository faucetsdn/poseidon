...
class Nodes:
    ...
    def scrape_prometheus(self):
        ...
        try:
            payload = {
                "query": "poseidon_endpoint_metadata",
                "start": start_time_str,
                "end": end_time_str,
                "step": "30s",
            }
            mr = httpx.get(
                "http://" + self.prometheus_addr + "/api/v1/query_range", params=payload
            )
            payload = {
                "query": "poseidon_role_confidence_top",
                "start": start_time_str,
                "end": end_time_str,
                "step": "30s",
            }
            ...
            payload = {
                "query": "poseidon_role_confidence_second",
                "start": start_time_str,
                "end": end_time_str,
                "step": "30s",
            }
            ...
            payload = {
                "query": "poseidon_role_confidence_third",
                "start": start_time_str,
                "end": end_time_str,
                "step": "30s",
            }
                ...
                if "result" in results["data"] and results["data"]["result"]:
                    for metric in results["data"]["result"]:
                        if metric["metric"]["hash_id"] in hashes:
                            if (
                                float(metric["values"][-1][1])
                                > hashes[metric["metric"]["hash_id"]]["latest"]
                            ):
...
class NetworkFull:
    ...
    def get_fields():
        return {
            ...
            "ipv4_rdns": NO_DATA,
            "ipv6_rdns": NO_DATA,
            "ether_vendor": NO_DATA,
            "controller_type": NO_DATA,
            "acls": NO_DATA,
        }
    ...
    def get_configuration():
        ...
        for field in Network.get_fields():
            configuration["fields"].append(
                {
                    "path": [field],
                    "displayName": Network.field_mapping()[field],
                    "groupable": "true",
                }
            )
