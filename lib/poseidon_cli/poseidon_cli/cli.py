...
class GetData:
    ...
    def _get_first_seen(endpoint):
        ...
        return (
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(endpoint.state_time))
            + " ("
            + duration(endpoint.state_time)
            + ")"
        )
...
class Parser:
    ...
    def completion(text, line, completions):
        ...
        completes = [
            s[offs:] for s in completions if s.lower().startswith(mline.lower())
        ]
    ...
    def display_results(
        ...
        if nonzero or unique:
            ...
            for endpoint in endpoints:
                raw_record = {
                    field: fields_lookup[field.lower()][0](endpoint) for field in fields
                }
                fields_with_data = {
                    field
                    for field, value in raw_record.items()
                    if value and value != NO_DATA
                }
...
class PoseidonShell(cmd2.Cmd):
    def __init__(self, *args, **kwargs):
        super().__init__(
            persistent_history_file="/opt/poseidon/.poseidon_history", *args, **kwargs
        )
...
/_/\n"""
        ...
        self.show_completions = [
            "role active-directory-controller",
            "role administrator-server",
            "role administrator-workstation",
        ...
        self.task_completions = ["set", "ignore", "remove", "collect", "clear"]
    ...
    def show_all(self, arg, flags):
        ...
        (
            ...
            ipv6_only,
            ipv4_and_ipv6,
        ) = self.parser._check_flags(flags, fields)
    ...
    def show_state(self, arg, flags):
        ...
        fields = ["Switch", "Port", "State", "Ethernet Vendor", "Mac", "IPv4", "IPv6"]
        ...
        (
            ...
            ipv6_only,
            ipv4_and_ipv6,
        ) = self.parser._check_flags(flags, fields, ipv4_only=False)
    ...
    def help_show(self):
        ...
        self.poutput("  history\tFind out the history of something on the network")
        self.poutput("  os\t\tShow devices matching a particular operating system")
    ...
    def show_authors(self, _arg, _flags):
        self.poutput(
            """\033[1;34m                            The Cyber Reboot Team
                                      &
  ...
  :yyyyys.       -syyyyyyyyys/.-:+ssyyyyyysso/:.:oyyyyyyyyys.       .syyyyy:
   /yyyyys.       `oyyyyyyyyyyys+:----------:/oyyyyyyyyyyy+`       .syyyyy/
    ...
    def help_task(self):
        ...
        self.poutput("  collect\tCollect on something on the network for a duration")
        self.poutput("  ignore\tIgnore something on the network")
        self.poutput("  remove\tRemove something on the network until it is seen again")
    ...
    def do_help(self, arg):
        if not arg:
            ...
            self.poutput("  show\t\t\tShow things on the network based on filters")
            ...
            self.poutput("  -unique\t\tRemoves columns that all contain the same value")
        else:
