...
class FaucetRemoteConfGetSetter:
    ...
    def __init__(
        self, client_key=None, client_cert=None, ca_cert=None, server_addr=None
    ):
    ...
    def get_stack_root_switch(self):
        root_stack_switch = [
            switch
            for switch, switch_conf in self.get_dps().items()
    ...
    def set_port_conf(self, dp, port, port_conf):
        return self.client.set_dp_interfaces([(dp, {port: yaml_dump(port_conf)})])
