...
class ACL:
    ...
    def include_acl_files(self, rules_doc, rules_file, coprocess_rules_files, obj_doc):
        ...
        for f in files:
            ...
            if isinstance(acls_doc, bool):
                self.logger.warning(
                    "Include file {0} was not found, ACLs may not be working as expected".format(
                        f
                    )
                )
                continue
        ...
        for f, acls_doc in acls_docs.items():
            ...
            if poseidon_acls_filename not in conf_files:
                ...
                self.frpc.write_faucet_conf(
                    os.path.join(rules_path, poseidon_acls_filename), acls_doc
                )
            ...
            if "rule" in r and "device_key" in r["rule"]:
                ...
                if rule_data["device_key"] == "os":
                    ...
                    for addresses in ("ipv4_addresses", "ipv6_addresses"):
                        if "addresses" in endpoint.metadata:
                            for ip, ip_metadata in endpoint.metadata["addresses"]:
                                if (
                                    "os" in ip_metadata
                                    and ip_metadata["os"] == rule_data["value"]
                                ):
                                    self.logger.info(
                                        "{0} os match: {1} {2}, rule: {3}".format(
                    ...
                    if "mac_addresses" in endpoint.metadata:
                        for mac, mac_metadata in endpoint.metadata[
                            "mac_addresses"
                        ].items():
                            most_recent = 0
                            ...
                            if (
                                most_recent != "0"
                                and most_recent in mac_metadata
                                and "labels" in mac_metadata[most_recent]
                                and "confidences" in mac_metadata[most_recent]
                            ):
                                # check top three
                                for i in range(3):
                                    if (
                                        mac_metadata[most_recent]["labels"][i]
                                        == rule_data["value"]
                                    ):
                                        if "min_confidence" in rule_data["value"]:
                                            if (
                                                float(
                                                    mac_metadata[most_recent][
                                                        "confidences"
                                                    ][i]
                                                )
                                                * 100
                                                >= rule_data["min_confidence"]
                                            ):
                                                self.logger.info(
                                                    "Confidence match: {0} {1}, rule: {2}".format(
                                                        mac,
                                                        float(
                                                            mac_metadata[most_recent][
                                                                "confidences"
                    ...
                    if match:
                        matches += 1
        if matches == len(rules[rule]) or (
            force_apply_rules and rule in force_apply_rules
        ):
            rule_acls = []
            for r in rules[rule]:
            ...
            if "acls_in" not in port_conf:
                self.logger.info(
                    "All rules met for: {0} on switch: {1} and port: {2}; applying ACLs: {3}".format(
    ...
    def apply_acls(
        ...
        if "include" not in rules_doc:
            ...
            obj_doc, acl_names = self.include_acl_files(
                rules_doc, rules_file, coprocess_rules_files, obj_doc
            )
        ...
        if "rules" in rules_doc:
            ...
            if "include" in rules_doc:
                for acl in acls:
                    if acl not in acl_names:
                        self.logger.info(
                            "Using named ACL: {0}, but it was not found in included ACL files, assuming ACL name exists in Faucet config".format(
                                acl
                            )
                        )
            ...
            for endpoint in endpoints:
                ...
                for acl in existing_acls:
                    if acl in acls and (
                        acl not in all_rule_acls or acl in force_remove_rules
                    ):
                        port_conf["acls_in"].remove(acl)
                        self.logger.info(
                            "Removing no longer needed ACL: {0} for: {1} on switch: {2} and port: {3}".format(
