...
class Monitor:
    ...
    def job_update_metrics(self):
        ...
        try:
            ...
            self.logger.error(
                "Unable to get current state and send it to Prometheus because: {0}".format(
                    str(e)
                )
            )
        return 0
    ...
    def _schedule_queued_work(
        self, queued_endpoints, budget, endpoint_state, endpoint_work, shuffle=False
    ):
        events = 0
        if self.sdnc.sdnc:
            if shuffle:
    ...
    def schedule_mirroring(self):
        ...
        queued_endpoints = [
            endpoint
            for endpoint in self.sdnc.not_ignored_endpoints("queued")
            if endpoint.operation_requested()
        ]
        queued_endpoints = sorted(queued_endpoints, key=lambda x: x.state_time)
        self.logger.debug(
            "operations {0}, budget {1}, queued {2}".format(
                str(self.sdnc.investigations), str(budget), str(len(queued_endpoints))
            )
        )
        return self._schedule_queued_work(
            queued_endpoints, budget, "trigger_next", self.sdnc.mirror_endpoint
        )
    ...
    def schedule_coprocessing(self):
        ...
        queued_endpoints = self.sdnc.not_copro_ignored_endpoints("copro_queued")
        queued_endpoints = sorted(queued_endpoints, key=lambda x: x.copro_state_time)
        self.logger.debug(
            "coprocessing {0}, budget {1}, queued {2}".format(
                str(self.sdnc.coprocessing), str(budget), str(len(queued_endpoints))
            )
        )
        return self._schedule_queued_work(
            queued_endpoints, budget, "copro_trigger_next", self.sdnc.coprocess_endpoint
        )
