    ...
    for worker in status:
        if "workers_state" in metrics:
            metrics["workers_state"].labels(
                worker=worker, state=status[worker]["state"]
            ).inc()
        else:
            metrics["workers_state"] = Gauge(
                "workers_state", "State of workers", ["worker", "state"]
            )
...
def callback(ch, method, properties, body, workers_json="workers.json"):
    ...
    for worker in workers["workers"]:
        ...
        if file_path in ["-1", ""]:
            print(
                " [X file empty] %s UTC %r:%r"
                % (str(datetime.datetime.utcnow()), method.routing_key, pipeline)
            )
        elif "id" in pipeline and (
            ("results" in pipeline and pipeline["results"]["tool"] in worker["inputs"])
            or ("file_type" in pipeline and pipeline["file_type"] in worker["inputs"])
        ):
            ...
            try:
                if use_swarm == "1":
                    ...
                    d.services.create(
                        ...
                        mounts=[vol_prefix + "/opt/poseidon_files:/files:rw"],
                        env=env,
                        args=command,
                    )
                else:
                    d.containers.run(
                        ...
                        volumes={
                            vol_prefix
                            + "/opt/poseidon_files": {"bind": "/files", "mode": "rw"}
                        },
                ...
                print(
                    " [Create container] %s UTC %r:%r:%r:%r"
                    % (
