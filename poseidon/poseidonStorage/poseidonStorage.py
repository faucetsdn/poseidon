...
class poseidonStorage:
    ...
    def __init__(self):
        ...
        database_container_ip = check_output("env | grep DOCKER_HOST | cut -d':' -f2 | cut -c 3-", shell=True).strip()
        if not database_container_ip:
            ...
            try:
                ...
                raise ValueError('poseidonStorage: could not find database ip address.')
