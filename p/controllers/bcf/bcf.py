...
from p.controllers.auth.cookie.cookieauth import CookieAuthControllerProxy
from p.controllers.mixins.jsonmixin import JsonMixin
...
class BcfProxy(JsonMixin, CookieAuthControllerProxy):
    ...
    def get_switches(
        ...
        self.poseidon_logger.debug('get_switches return:{0}'.format(retval))
        return retval
    ...
    def get_tenants(
        ...
        self.poseidon_logger.debug(sout)
        return retval
    ...
    def shutdown_ip(self, ip_addr, shutdown=True, mac_addr=None):
        ...
        for record in records:
            ...
            if record.get('name') is not None:
                name = record['name']
            self.poseidon_logger.debug('bcf shutting down: {0}'.format(record))
            self.poseidon_logger.debug('t:{0} s:{1} n:{2} m{3} shut:{4}'.format(
    ...
    def get_highest(span_fabric):
        ...
        if my_filter is not None:
            ...
            self.poseidon_logger.debug('noFilters online')
            return 1
    ...
    def mirror_traffic(
        ...
        self.poseidon_logger.debug('{0}'.format(data))
        if mirror:
            new_filter = {}
